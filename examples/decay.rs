...
fn decay_rhs(y: &f64, _t: &f64, p: &[f64; 1]) -> Result<f64,PyErr> {
    return Ok(- p[0] * y);
}


fn solve_ode(y0: &f64, t_start: &f64, t_end: &f64, dt: &f64, p: &[f64; 1]) -> (Vec<f64>, Vec<f64>) {
    let mut t = *t_start;
...
fn main() {
    ...
    let t_start = 0.0;
    let t_end   = 100.0;
    let dt_all  = [0.5, 0.1, 0.05, 0.01, 0.005, 0.001, 0.0005, 0.0001];//, 0.00005, 0.00001];
